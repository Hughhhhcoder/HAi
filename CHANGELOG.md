...
### 🤖 AI 专业报告 + 实时画像更新
- **AI 自动生成专业评估报告**：每次测评完成后自动调用 AI 生成详细的心理评估报告
  - 专业资深心理学家角色定位
  - 包含：测评结果总结、详细解读、心理健康建议、积极资源与优势
  - 基于国际诊断标准（DSM-5、ICD-11）
  - 温暖、客观、实用的专业建议
  - 伦理声明：强调为筛查工具，非临床诊断
- **测评结果实时同步到用户画像**：
  - 自动提取测评关键洞察（困扰、优势、人际模式等）
  - 更新 `user_insights` 表（main_concerns, strengths, coping_patterns, triggers等）
  - AI 对话时自动读取最新测评状态，提供更个性化的支持
  - 示例：中重度抑郁 → 画像记录"抑郁(中重度)" → AI 对话时自动感知
- **新增后端服务**：
  - `backend/app/services/report_service.py`：AI 报告生成服务
  - 支持 httpx 异步调用外部 AI API
  - 智能降级：AI 不可用时返回基础建议
  - 超时控制（60秒）和错误处理
- **新增数据库字段**：
  - `psych_tests.ai_report`（TEXT）：存储 AI 生成的专业报告
- **前端 UI 升级**：
  - 测评结果页新增「AI 专业评估报告」专区
  - 渐变紫蓝色卡片设计，突出专业性
  - Markdown 渲染支持（标题、粗体、列表）
  - 伦理声明提示
- **用户画像增强**：
  - `memory_service` 新增 `update_profile_with_psych_test` 函数
  - 根据测评类型智能提取洞察（10种量表支持）
  - 提取积极资源：共情能力强、自尊感良好等
