...
- **专业 Markdown 渲染**：支持标题、列表、强调文本、代码块等格式，提供最佳可读性
- **用户体验优化**：提交时显示加载状态，避免用户等待焦虑
- **AI聊天Markdown支持**：AI回复支持完整的Markdown格式，包含标题层次、强调文本、代码块、引用等
...
### 📊 历史记录摘要系统（NEW）
- **AI对话历史记录**：
  - 完整对话历史查看，支持用户/AI消息类型标识
  - 智能时间格式化（今天/昨天/完整日期显示）
  - 总对话数和最近对话时间统计
  - 美观的弹窗设计，支持悬停效果和过渡动画

- **心理测评记录摘要**：
  - 历史测评数据完整展示，包含分数、等级、时间戳
  - 总测评数和最近测评时间统计
  - 按时间倒序排列，最新记录优先显示
  - 专业的测评记录弹窗设计

- **每日打卡记录摘要**：
  - 历史打卡记录详细展示，包含心情、睡眠、完成事项
  - 总打卡数和最近打卡时间统计
  - 支持详细的时间戳和日期信息显示
  - 清晰的数据统计和摘要展示
